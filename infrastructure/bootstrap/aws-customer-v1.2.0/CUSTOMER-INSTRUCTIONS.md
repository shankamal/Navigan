...
Bootstrap package version: **1.2.0**
...
10. When private connector installation is enabled, return the
    `connector_installer_project_name` and `connector_installer_role_arn`
    outputs. Navigan records these non-secret identifiers in the approved
...
- The connector installer uses only approved private subnets and security
  groups, has no inbound listener, and can read only ephemeral secrets under
  `navigan/connectors/`.
